...
class UsersController extends Controller
{
    ...
    public function home(){
        ...
        if (!isset($_COOKIE['UserIdToDoSmarter'])) {
    ...
    public function viewMessages(){
    	...
    	$data = DB::select("SELECT * FROM users_chat,users WHERE (users_chat.chat_to_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' OR users_chat.chat_from_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."') AND users_chat.chat_from_id=users.id AND users_chat.chat_date_time IN (SELECT MAX(chat_date_time) FROM users_chat GROUP BY chat_from_id) ORDER BY users_chat.chat_date_time DESC");
    ...
    public function viewAccount(){
       ...
       $data = User::where('id', '=', [Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])])->get();
     ...
     public function changePasswordAction(Request $request){
        ...
        $data = User::where('id', '=', [Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])])->get();
            ...
            if(md5($request->input('curretpswrd')) == $data[0]->password){
                     ...
                     User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['password' => md5($request->input('newpswrd'))]);
    ...
    public function dashboard(){
	   ...
	   $data = DB::table('users')
                ->leftJoin('users_metas', 'users_metas.user_id', '=', 'users.id')
                ->where('users.id', '!=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->orderBy('users.id', 'DESC')
                ->get();

       $photo = User::where('id', '=', [Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])])->get();

       $user = DB::select("SELECT * FROM connectionsfinals WHERE user_id= '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");
        

        foreach ($user as $use) {

          $this->mutual[] =  DB::select("SELECT t1.*,t2.*,t3.* FROM connectionsfinals t1 LEFT JOIN users_metas t2 ON (t1.user_id = t2.user_id) LEFT JOIN users t3 ON (t1.user_id = t3.id) WHERE t1.user_id = '".$use->you_like."' AND t1.you_like = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");
...
WHERE `users`.`id` != '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND `users_metas`.`sex` = '".Crypt::decryptString($_COOKIE['LookingFor'])."'  AND `users`.`prfl_approve_status` = '1' AND `users`.`id` NOT IN ((
SELECT b_who_id
FROM block_users
WHERE b_whom_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'))
...
// ORDER BY `users`.`id` DESC LIMIT $offset,$limit");
	   ...
	   if (isset($_REQUEST['distance'])){
			$data = DB::table('users')
                ...
                ->whereNotIn('users.id', DB::table('block_users')->select('b_who_id')->where('b_whom_id','=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])))
    ...
    public function onlineuser(Request $request){
       ...
       $totalusers =  DB::select("SELECT *
        ...
        LEFT JOIN `users_metas` ON `users_metas`.`user_id` = `users`.`id`
        LEFT JOIN `users_match` ON `users_match`.`users_match_id` = `users`.`id`
        WHERE `users`.`id` != '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND `users_metas`.`sex` = '".Crypt::decryptString($_COOKIE['LookingFor'])."' AND `users`.`prfl_approve_status` = '1' AND `users`.`onln_status` = 'online' AND `users`.`id` NOT IN (
        SELECT b_who_id
        FROM block_users
        WHERE b_whom_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."')");
    ...
    public function userProfile($id){
       ...
       DB::table('notifications')->insert([
            'notifications_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']),
    ...
    //             ->get();
       ...
       //          ->get();    
        ...
        $users = Connectionsfinal::where('user_id', '=', [Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])])
                    ->where('you_like', '=', [$id])
                    ->get();

        $view_profile= DB::select("SELECT * FROM view_profile WHERE pro_user_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND view_user_id = '".$id."'");
        $report_profile= DB::select("SELECT * FROM report_users WHERE r_user_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND r_who_id = '".$id."' and (r_status = 0 or r_status = 2) ");
        if(count($view_profile) == 0){

            DB::table('view_profile')->insert([
            'pro_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), ///je dekhche
    ...
    public function profilesettingspost(Request $request){
		...
		User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['onln_status' => $request->input('onlnstts'), 'display_status' => $request->input('disstts'), 'offline_message' => $request->input('offline_message'), 'subscribe_newsletter' => $request->input('subscribe_newsletter'), 'hide_status' => $request->input('hide_photos')]);
    ...
    public function viewPhotos(){
	    ...
	    $data = DB::table('user_photos')
                ...
                ->where('user_photos.user_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->orderBy('user_photos.created_at', 'desc')
                ->get();

        $email= DB::Select('SELECT email_verified_at FROM users WHERE id= "'.Crypt::decryptString($_COOKIE['UserIdToDoSmarter']).'"');
    ...
    public function photoUploadPost(Request $request){
        ...
        DB::table('profile_post_photos')->insert([
            'pro_pic_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']),
    ...
    public function uploadPost(Request $request){
       ...
       ->get();
        ...
        file_put_contents($imageFullPath, $image_base64);
            ...
            $user_photos->user_id  = Crypt::decryptString($_COOKIE['UserIdToDoSmarter']);
            $user_photos->user_photo_path  = 'https://www.globallove.online/images/' . $imageName;
            $user_photos->save();

            User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['prfl_approve_status'=> '0']);
    ...
    public function prflPhotoAction($title){
       ...
       User::where('id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['prfl_photo_path' => 'https://www.globallove.online/images/' . $title, 'prfl_approve_status'=> '1']);
    ...
    public function viewBlocklist(){
        ...
        $user = DB::table('block_users')
                ...
                ->leftJoin('users_metas', 'users_metas.user_id', '=', 'users.id')
                ->where('block_users.b_who_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
     ...
     public function viewConnection(){
        ...
        $user = DB::table('connectionsfinals')
                ...
                ->leftJoin('users_metas', 'users_metas.user_id', '=', 'users.id')
                ->where('connectionsfinals.user_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->where('users.prfl_photo_path', '!=', 'https://www.globallove.online/images/male-user-placeholder.png')
                ->whereNotIn('users.id', DB::table('block_users')->select('b_who_id')->where('b_whom_id','=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])))
    ...
    public function viewLikes(){
        ...
        $user = DB::table('connectionsfinals')
                ...
                ->leftJoin('users_metas', 'users_metas.user_id', '=', 'users.id')
                ->where('connectionsfinals.you_like', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->where('users.prfl_photo_path', '!=', 'https://www.globallove.online/images/male-user-placeholder.png')
                ->whereNotIn('users.id', DB::table('block_users')->select('b_who_id')->where('b_whom_id','=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])))
                ->orderBy('connectionsfinals.created_at', 'desc')
                ->get();
...
public function viewProfile(){
    ...
    if(!$this->isUserLoginExist()){
        ...
        $heading = DB::table('users_metas_status')
                ->where('users_metas_status.user_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->where('users_metas_status.metas_field_name', '=', 'heading')
                ->orderBy('users_metas_status.id','DESC')
                ->take(1)
                ->get();
       ...
       $about = DB::table('users_metas_status')
        ...
        $photopost = DB::table('profile_post_photos')
        ->where('profile_post_photos.pro_pic_user_id', '=',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
        ->orderBy('profile_post_photos.pro_pic_id','DESC')
        ->get();   
        
        $videopost = DB::table('profile_videos')
        ->where('profile_videos.pro_vid_user_id', '=',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
        ->orderBy('profile_videos.pro_vid_id','DESC')
        ->get();   
    ...
    public function verifyProfileAction(Request $request){
        ...
        $request->image->move(public_path('images/verify_img'), $imageName);
            ...
            User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['verify_photo_path'=> 'https://www.globallove.online/images/verify_img/' . $imageName]);
    ...
    ->where('users_metas.user_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
    ->get();
 	
 
 	User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['name' => $request->input('name')]);

 	UsersMeta::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['country' => $request->input('country'), 'city' => 'All', 'state' => 'All', 'hair_color' => $request->input('hairclr'),'eye_color' => $request->input('eyeclr'), 'height' => $request->input('height'), 'weight' => $request->input('weight'), 'body_type' => $request->input('bdytyp'), 'ethnicity' => $request->input('ethnicity'), 'body_art' => $request->input('bdyart'),'appearance' => $request->input('apprnce'), 'drink' => $request->input('drink'), 'smoke' => $request->input('smoke'),'relationship' => $request->input('rltnshp'), 'occupation' => $request->input('occptn'), 'emplyment_status' => $request->input('empstatus'), 'annual_income' => $request->input('income'), 'nationality' => $request->input('ntnality'),'education' => $request->input('education'), 'language_spoke' => $request->input('langspoke'), 'eng_lang_ability' => $request->input('engablty'),'religion' => $request->input('religion'), 'religious_view' => $request->input('rlgsval'), 'star_sign' => $request->input('starsign'), 'heading' => $request->input('prflhdng'), 'about' => $request->input('about'), 'prtnr_typ_desc' => $request->input('prtnrdesc')]);
      ...
      //dd($usersmeta);
        ...
        if($usersmeta[0]->heading != $request->input('prflhdng')){
            ...
            $UsersMetaheading = UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','heading')->first();

            if ($UsersMetaheading === null) {

                DB::table('users_metas_status')->insert([
                    'user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), 
                    ...
                    UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','heading')->update(['metas_old_value' => $usersmeta[0]->heading,'metas_new_value' => $request->input('prflhdng'),'status' => 0]);
                }
        ...
        if($usersmeta[0]->about != $request->input('about')){
            ...
            $UsersMetaabout = UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','about')->first();
            ...
            if ($UsersMetaabout === null) {
                ...
                DB::table('users_metas_status')->insert([
                    ...
                    UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','about')->update(['metas_old_value' => $usersmeta[0]->heading,'metas_new_value' => $request->input('about'),'status' => 0]);

                }
        }  
        
        
        $UsersMetaprtnr_typ_desc = UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','prtnr_typ_desc')->first();
        ...
        if ($UsersMetaprtnr_typ_desc === null) {
            ...
            if($usersmeta[0]->prtnr_typ_desc != $request->input('prtnrdesc')){
                UsersMetaStatus::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->where('metas_field_name','prtnr_typ_desc')->update(['metas_old_value' => $usersmeta[0]->prtnr_typ_desc,'metas_new_value' => $request->input('prtnrdesc'),'status' => 0]);
             }   
        }
    ...
    public function viewProfileMatch(){
        ...
        if(!$this->isUserLoginExist()){
            ...
            $data = DB::table('users_match')
                ->where('users_match_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->get();
    ...
    public function profileMatchAction(Request $request){
        ...
        $data= DB::select("SELECT * FROM users_match WHERE users_match_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");

        if (count($data) == 0) {

            
            DB::table('users_match')->insert([
            'users_match_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), 
            ...
            DB::table('users_match')->where('users_match_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']) )->update([
            'users_match_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), 
            'match_seeking' => $request->input('seeking'),
            'match_min_age' => $request->input('min_age'), 
            'match_max_age' => $request->input('max_age'), 
...
setcookie('LookingFor', Crypt::encryptString($request->input('seeking')), time() + (86400 * 365), "/");
    ...
    public function viewProfileUsers(){
        ...
        $data = DB::table('users')
                ...
                ->where('view_profile.view_user_id', '=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                ->whereNotIn('users.id', DB::table('block_users')->select('b_who_id')->where('b_whom_id','=', Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])))
 ...
 public function profileDetailsAction(Request $request){
    ...
    if(!$this->isUserLoginExist()){
 	...
 	UsersMeta::where('user_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['entertainment' => $entertainment, 'food' => $food, 'music' => $music, 'sports' => $sports]);
    ...
    public function viewPersonalityQuestion(){
        ...
        $data= DB::select("SELECT * FROM users_prsnl_qstn WHERE prsnl_user_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");
    ...
    public function persnlquestnactn(Request $request){
        ...
        if (count($data) == 0) {
            ...
            'prsnl_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), 
            ...
            DB::table('users_prsnl_qstn')->where('prsnl_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']) )->update(['prsnl_movie' => $request->input('movie'),
            'prsnl_book' => $request->input('book'), 
            'prsnl_food' => $request->input('food'), 
            'prsnl_music' => $request->input('music'), 
            'prsnl_hobbies' => $request->input('hobbies'), 
    ...
    public function viewChangeEmail(){
        ...
        $data= DB::select("SELECT * FROM users WHERE id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");
    ...
    public function changeEmailAction(Request $request){
        ...
        if(count($data) == 0) {
            ...
            User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['email' => $request->input('email')]);
    ...
    public function proVideoUploadPost(Request $request){
        ...
        DB::table('profile_videos')->insert([
            'pro_vid_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']),
    ...
    public function profilePhotoAction(Request $request){
       ...
       $data= DB::Select('SELECT email_verified_at FROM users WHERE id = "'.Crypt::decryptString($_COOKIE['UserIdToDoSmarter']).'"');
       ...
       if($data[0]->email_verified_at != ''){
        ...
        User::where('id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['prfl_photo_path' => 'https://www.globallove.online/images/' . $imageName, 'prfl_approve_status'=> '0']);
        ...
        if(isset($_POST['submit'])){ 
            ...
            if($reportcount > 0){
                ...
                $newid = UsersReport::insert([
                    'r_user_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']), 
 ...
 public function mobprofileDetailsAction(Request $request){
    ...
    public function ajaxNotifUpdate(){
        ...
        //dd("xxx");
	   Notifications::where('notifications_to_id',Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['status' => '1']);
...
$totalusers= $totalusers;
    ...
    public function preferredLanguage() {
        ...
        $data= DB::table('users')->where('id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
        ->get();
    ...
    public function preferredLanguagePost(Request $request) {
        ...
        User::where('id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update(['users_lang' => $request->input('language')]);
    ...
    public function deleteProfileActionUser(Request $request) {
      ...
      $data = DB::select("SELECT * FROM users WHERE id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND password = '".md5(request('curretpswrd'))."'");
        if(count($data) == '1'){
        DB::table('block_users')->where('b_whom_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->delete();

        DB::table('connectionsfinals')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('you_like', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('notifications')->where('notifications_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('notifications_to_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('payment_transactions')->where('pt_u_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->delete();

        DB::table('report_users')->where('r_who_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('r_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        $chat_files= DB::table('users_chat')->where('chat_from_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('chat_to_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
        ...
        DB::table('users_chat')->where('chat_from_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('chat_to_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('users_chat_rooms')->where('room_from_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->orwhere('room_to_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('users_match')->where('users_match_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('users_metas_status')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('users_prsnl_qstn')->where('prsnl_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        $user_phto_img= DB::table('user_photos')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->get();
        ...
        DB::table('user_photos')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                      ->delete();

        DB::table('view_profile')->where('pro_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                 ->orwhere('view_user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                  ->delete();

        DB::table('users_metas')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                ->delete();

        DB::table('users')->where('id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
                                ->delete();
    ...
    public function tinder($id) {
        $arr = [];
        //$sql = DB::select("select * from users where id != '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'");
        ...
        LEFT JOIN `users_metas` ON `users_metas`.`user_id` = `users`.`id`
        LEFT JOIN `users_match` ON `users_match`.`users_match_id` = `users`.`id`
        WHERE `users`.`prfl_photo_path` != 'https://www.globallove.online/images/male-user-placeholder.png' AND users.display_status = 'show' AND `users`.`id` != '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."' AND `users_metas`.`sex` = '".Crypt::decryptString($_COOKIE['LookingFor'])."'  AND `users`.`prfl_approve_status` = '1' AND `users`.`id` NOT IN ((
        SELECT b_who_id
        FROM block_users
        WHERE b_whom_id = '".Crypt::decryptString($_COOKIE['UserIdToDoSmarter'])."'))
        ORDER BY `users`.`id` DESC");
        ...
        return view('users.tinder', ['next' => $next, 'query' => $query]);
    }
}
