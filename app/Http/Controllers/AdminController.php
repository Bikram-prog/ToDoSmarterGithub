...
class AdminController extends Controller
{
  ...
  public $extn;
    ...
    public function GetPostAThought()
    {
      $data['status'] = true;
      $data['msg'] = "success";
  ...
  $arr=[];
    ...
    $notes= DB::table('admin_to_do_list_bes')->where('to_do_list_id', $request->input('to_do_id'))->get();
      ...
      $mail= $data[0]->reciepent_email;
        ...
        $message->to($mail)->subject
           ('You have a new updated job - ToDoList');
        $message->from('no-reply@wwwmedia.world','Bescrow');
      });
...
public function adminApp(){
  ...
  return view('admin.adminapp');
   ...
   ///////////////////////// To Do Smarter /////////////////////////////
   public function toDoSmarterHome(){
    if (isset($_COOKIE['UserFullNameToDoSmarter'])) {
      return redirect('/u/to-do-list-smarter');
     }

     $glogin=$this->googleLoginAction();

     return view('admin.todosmarterhome')->with(['glogin'=> $glogin]);
 }

 public function addToDoListReceiepent(){

  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
    return redirect('/');
   }

 return view('admin.add-reciepent');
}

public function addReciepentPost(Request $request){
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
    return redirect('/');
   }
...
[
    'title'=> 'Full Name field is required', // custom message
    'email'=> 'Email field is required', // custom message
    'password'=> 'Password field is required', // custom message
]);
   ...
   DB::table('to_do_list_reciepent')->insert([
    ...
    'reciepent_company_email' => Crypt::decryptString($_COOKIE['UsersCompanyEmailToDoSmarter']),
    'reciepent_company_cntct_no' => Crypt::decryptString($_COOKIE['UsersCompanyContactToDoSmarter']),
    'reciepent_company_address' => Crypt::decryptString($_COOKIE['UsersCompanyAddressToDoSmarter']),
    'reciepent_company_logo' => Crypt::decryptString($_COOKIE['UsersCompanyLogoToDoSmarter']),
...
Mail::send(['html'=>'admin.admin_todo_rec_smarter_mail'], ['name'=>$request->input('title'), 'email'=> $request->input('email'), 'psswrd'=> $request->input('password'),'text'=>$dataa], function($message) use ($email)  {
 ...
 return redirect('/u/all_reciepent')->with(['msg' => 'Reciepent Added Successfully.']);

//  return response()->json(['msg'=>'Reciepent added successfully']);
}

public function addToDoListNew(){
 ...
 return view('admin.add-todo-list')->with(['data'=>$data]);
}

 public function toDoSmarterPrivacy(){

  return view('privacy');
}
public function toDoSmarterTerms(){

  return view('terms');
}

public function toDoSmarterCookies(){
...
public function toDoSmarterNews(){
...
return redirect('finish/'. $last)->with(['user_id', $last]);
   ...
   public function toDoSmarterLogin(){
    if (isset($_COOKIE['UserFullNameToDoSmarter'])) {
     ...
     return view('admin.to_do_smarter_login')->with(['glogin'=> $glogin]);
 }
 ...
 public function toDoSmarterLogPost(Request $request){
  ...
  if(count($data) == '1'){
    ...
    if($data[0]->email_verified_at == ""){
                return redirect('/u/to-do-smarter-login')->with('regsucmsg','Please verify your email first to be able to login.');
            }
  ...
  setcookie('UsersCompanyEmailToDoSmarter', Crypt::encryptString($data[0]->reciepent_company_email), time() + (86400 * 365), "/");
  setcookie('UsersCompanyContactToDoSmarter', Crypt::encryptString($data[0]->reciepent_company_cntct_no), time() + (86400 * 365), "/");
  setcookie('UsersCompanyAddressToDoSmarter', Crypt::encryptString($data[0]->reciepent_company_address), time() + (86400 * 365), "/");
  setcookie('UsersCompanyLogoToDoSmarter', Crypt::encryptString($data[0]->reciepent_company_logo), time() + (86400 * 365), "/");
      return redirect('/u/to-do-list-smarter');
  }else{
...
public function toDoListSmarter($id=null){
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
  ...
  $open = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', '0')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->get();

  $progress = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', 'In Progress')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->get();

  $delay = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', 'Delay')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->get();

  $completed = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', '1')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->get();

  if ($id == 'open'){
    $data = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', '0')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->paginate(15);
  }elseif ($id == 'in-progress'){
    $data = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', 'In Progress')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->orderBy('to_do_list_datetime', 'desc')->paginate(15);
  }elseif ($id == 'delay'){
    $data = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', 'Delay')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->paginate(15);
  }elseif ($id == 'completed'){
    $data = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', '1')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->paginate(15);
  }else{
    $data = DB::table('to_do_list_new')->where('to_do_list_new.to_do_list_status', '!=', '1')->where('to_do_list_new.to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->where('to_do_list_new.to_do_list_job_status', '=', '1')->orderBy('to_do_list_datetime', 'desc')->paginate(15);
  }

   return view('admin.admin_to_do_list_smarter_new')->with(['data'=>$data,'open'=>$open,'delay'=>$delay,'progress'=>$progress,'completed'=>$completed, '_id_stat' => $id]);
}

    private function date_range($first, $last, $step = '+1 day', $output_format = 'D j' ) {
        $arr_holidays = [];

        $holidays = DB::table("job_rating_holidays")
            ->where("company_id", "=", Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))
            ->get();

        foreach ($holidays as $h) {
            $arr_holidays[] = date('D j', strtotime($h->holidays));
        }
        ...
        //stack
        $valuesCounterArr = array_count_values($dates);
$finalArr = [];
...
public function jobDetailsView($id){
  ...
  DB::table('to_do_list_new_seen')->where('user_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']) )->where('job_id', $id )->update(['status' => '1']);


    $job_rating_progress = DB::table("job_rating")
        ->where("job_id", "=", $id)
        ->where("reipent_id", "=", Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))
        ->where("status", "=", 0)
        ->first();
    ...
    if($job_rating_completed !== null) {
        return view('admin.job-details')->with(['data'=> $data, 'rate' => 1, 'company_settings' => $company_settings, 'job_rating_progress' => $job_rating_progress, 'job_rating_completed' => $job_rating_completed, 'date_range' => $this->date_range($job_rating_progress->datetime, $job_rating_completed->datetime)]);
    } else {
        return view('admin.job-details')->with(['data'=> $data, 'rate' => '0']);
    }


}

public function allReceiepent(){
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
    return redirect('/u/to-do-list-smarter');
   }
  ...
  return view('admin.all-reciepent')->with(['data'=> $data]);
...
public function delToDoSmarter($id){
  ...
  if (Crypt::decryptString($_COOKIE['UserStatusToDoSmarter']) == '0') {
    return response()->json(['msg'=>'You are not able to delete to do list.']);
      ...
      DB::table('to_do_list_new_seen')->where('job_id', $id)->delete();

      DB::table('todo_job_months')->where('m_job_id', $id)->delete();

      $data= DB::table('to_do_list_new_attachment')->where('attach_job_id', $id)->get();

      DB::table('to_do_list_new_attachment')->where('attach_job_id', $id)->delete();

      DB::table('job_rating')->where('job_id', $id)->delete();

      foreach($data as $da){
        $path=public_path().'/images/to_do_smarter_img'.$da->attach_path;
        if (file_exists($path)) {
        unlink($path);
        }
      }
     ...
     return redirect('/u/to-do-list-smarter')->with(['msg' => 'Job Deleted Successfully.']);
 }
 ...
 public function inProgressToDoActionSmarter($id){
    ...
    DB::table('job_rating')->insert([
      'job_id' => $id,
      'reipent_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']),
     ...
     //return response()->json(['msg'=>'Reciepent deleted successfully']);
     return redirect('/u/to-do-list-smarter')->with(['msg' => 'Status Changed Successfully.']);
 }
...
public function delReciepentSmarter($id){
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
   ...
   $data=DB::table('to_do_list_new')->where('to_do_list_add_id', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->get();

   $dataname=DB::table('to_do_list_reciepent')->where('reciepent_id', $id)->get();
...
if($impname == ''){
  $impname= Crypt::decryptString($_COOKIE['UserFullNameToDoSmarter']);
}
...
foreach($expId as $exi){
  if($exi == $id){
    foreach (array_keys($expId, $id) as $key) {
      unset($expId[$key]);
...
if($impid == ''){
  $impid= Crypt::decryptString($_COOKIE['UserIdToDoSmarter']);
  ...
  DB::table('to_do_list_new')->where('to_do_list_id', $da->to_do_list_id)->update(['to_do_list_reciepent' => $impname.'|'.$impid]);
      ...
      DB::table('to_do_list_new_seen')->where('user_id', $id)->delete();
      DB::table('to_do_list_reciepent')->where('reciepent_id', $id)->delete();

     //return response()->json(['msg'=>'Reciepent deleted successfully']);
     return redirect('/u/all_reciepent')->with(['msg' => 'Reciepent Deleted Successfully.']);
 }
...
//  });
    ...
    return redirect('/u/add-attachment/'.$last);
      // return redirect('/u/to-do-list-smarter');
      // return response()->json(['msg'=> 'Job uploaded successfully']);
 }
 ...
 public function delProductImage(Request $request, $id, $prod_id, $imgpath){
  ...
  DB::table('to_do_list_new_attachment')->where('attach_id', $id)->delete();
  $path=public_path().'/images/to_do_smarter_img'.$imgpath;
if (file_exists($path)) {
 unlink($path);
}

return redirect('/u/add-attachment/'. $prod_id);
 }

 public function liveProduct($id){

  DB::table('to_do_list_new')->where('to_do_list_id', $id )->update(['to_do_list_job_status' => '1']);
  ...
  foreach ($expnew as $idd) {
    $datac = DB::table('to_do_list_reciepent')
            ->where('reciepent_id', '=', $idd)
            ->get();

    $mail= $datac[0]->reciepent_email;
    $title= $data[0]->to_do_list_title;

    Mail::send(['html'=>'admin.todolist_smarter_mail'], ['text'=>$title], function($message) use ($mail)  {
  $message->to($mail)->subject
     ('You have a new job - ToDoList');
  $message->from('no-reply@wwwmedia.world','To Do Smarter');
});
 ...
 public function imgUpload($id){
      ...
      $ProImage = DB::table('to_do_list_new_attachment')
                                      ->where('attach_job_id', '=', $id)
                                      ->get();

      $imgCount= count($ProImage);
      $uploadCount= 5-$imgCount;

      $image = count($ProImage);

    return view('admin.upload_attachment')->with(['lstprodid' => $id,'ProImage'=>$ProImage, 'image'=>$image,'uploadCount' =>$uploadCount]);
}

public function fileStore(Request $request){

  $jobattach = DB::table('to_do_list_new_attachment')->where('attach_job_id', '=', $request->input('prod_id'))->get();


  $attachCount= count($jobattach);

if($attachCount < 5){
$jobid = $request->input('prod_id');
...
public function ProdImage($id){
  ...
  $ProImage = DB::table('to_do_list_new_attachment')->where('attach_job_id', '=', $id)->get();
 ...
 public function editToDoListActionSmarter(Request $request){
  ...
  $arr=[];
    ...
    return redirect('/u/add-attachment/'.$request->input('to_do_id'));
    //  return redirect('/u/to-do-list-smarter');
 }
...
public function toDoSmarterLogout() {
  ...
  setcookie("UsersCompanyEmailToDoSmarter", "", time() - 3600, "/");
  setcookie("UsersCompanyContactToDoSmarter", "", time() - 3600, "/");
  setcookie("UsersCompanyAddressToDoSmarter", "", time() - 3600, "/");
  setcookie("UsersCompanyLogoToDoSmarter", "", time() - 3600, "/");
...
// init configuration
$clientID = '332462761689-ac0h5mvgi52kg8b44v6fj5c0hbd0lu1g.apps.googleusercontent.com';
$clientSecret = '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';
$redirectUri = 'https://todosmarter.com/g-login-redirect';
...
$data_user= DB::Select("SELECT * FROM to_do_list_reciepent WHERE reciepent_email = '".$email."'");
...
return redirect('finish/'. $last)->with(['user_id', $last]);
  ...
  setcookie('UserIdToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_id), time() + (86400 * 365), "/");
  setcookie('UserEmailToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_email), time() + (86400 * 365), "/");
  setcookie('UserFullNameToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_name), time() + (86400 * 365), "/");
  setcookie('UserStatusToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_status), time() + (86400 * 365), "/");
  setcookie('UsersCompanyToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_company_name), time() + (86400 * 365), "/");
  setcookie('UsersCompanyEmailToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_company_email), time() + (86400 * 365), "/");
  setcookie('UsersCompanyContactToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_company_cntct_no), time() + (86400 * 365), "/");
  setcookie('UsersCompanyAddressToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_company_address), time() + (86400 * 365), "/");
  setcookie('UsersCompanyLogoToDoSmarter', Crypt::encryptString($data_user[0]->reciepent_company_logo), time() + (86400 * 365), "/");
...
public function finish($user_id){
  ...
  return view('admin.finish')->with(['user_id'=> $user_id]);

}

public function setupCompanyView(){

return view('admin.setup_company');
}

public function setupCompanyPost(Request $request){

  DB::table('to_do_list_reciepent')->where('reciepent_company_name', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->update([
    'reciepent_company_email' => $request->input('comp_email'),
    'reciepent_company_cntct_no' => $request->input('comp_number'),
    'reciepent_company_address' => $request->input('comp_address')
    ]);

  return redirect('/u/to-do-list-smarter');
 }

 public function makeAdminPost($id){
  ...
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
   ...
   if (Crypt::decryptString($_COOKIE['UserStatusToDoSmarter']) == '0') {
    return response()->json(['msg'=>'You are not able to make admin reciepent.']);
   }

   DB::table('to_do_list_reciepent')->where('reciepent_id', $id)->update([
    'reciepent_status' => '1'
    ]);

    return redirect('/u/all_reciepent');
 }

public function editReciepentView($id){
  ...
  //  if (Crypt::decryptString($_COOKIE['UserStatusToDoSmarter']) == '0') {
  //   return response()->json(['msg'=>'You are not able to edit reciepent.']);
  //  }

    $data= DB::table('to_do_list_reciepent')->where('reciepent_id', $id)->get();

    return view('admin.edit_to_do_reciepent')->with(['data'=>$data]);
 }
public function editReciepentPost(Request $request){
...
[
  ...
  DB::table('to_do_list_reciepent')->where('reciepent_id', $request->input('rec_id') )->update([
    ...
    Mail::send(['html'=>'admin.admin_todo_rec_smarter_mail'], ['name'=>$request->input('title_rec'), 'email'=> $request->input('email_rec'), 'psswrd'=> $request->input('psw_rec')], function($message) use ($email)  {
      $message->to($email)->subject
         ('You are updated as a reciepent - ToDoList');
      $message->from('no-reply@wwwmedia.world','To Do Smarter');
    });
...
public function finishPost(Request $request){
  ...
  DB::table('to_do_list_reciepent')->where('reciepent_id', $request->input('rec_id') )->update([
    ...
    'reciepent_company_logo' => $imageName
    ]);

    $data_user= DB::Select("SELECT * FROM to_do_list_reciepent WHERE reciepent_id = '".$request->input('rec_id')."'");

    if($data_user[0]->email_verified_at == ''){
      $mail= $data_user[0]->reciepent_email;

        $dataa = "https://todosmarter.com/verifyaccount/" . $data_user[0]->reciepent_id;
      ...
      return redirect('/u/to-do-smarter-login')->with(['regsucmsg'=> 'Please Verify Your E-mail For Login']);
...
public function viewVerifyAccount($id){
  ...
  DB::table('to_do_list_reciepent')->where('reciepent_id', $id)->update([
    'email_verified_at' => $day
    ]);

    return redirect('/u/to-do-smarter-login')->with(['regsucmsg'=> 'E-mail Verified Successfully']);
}

public function excel()
    {

     $customer_data = DB::select('select * from to_do_list_new where to_do_list_add_id  =? ',[Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter'])]);
     $customer_array[] = array('Job Title', 'Description', 'Deadline', 'Recipient Name', 'Status');
     foreach($customer_data as $customer)
     {
      $customer_array[] = array(
       'Job Title'  => $customer->to_do_list_title,
       'Description'   => $customer->to_do_list_desc,
       'Deadline'    => $customer->to_do_list_target_days,
       'Recipient Name'  => $customer->to_do_list_reciepent,
       'Status'   => $customer->to_do_list_status
      );
     }
     ...
     Excel::store('Customer Data', function($excel) use ($customer_array){
      ...
      // return Excel::download(new Todolistnew, 'disney.xlsx');
}

public function profilePhotoAction(Request $request){

  $folderPath = public_path('images/to_do_smarter_img/');
  ...
  DB::table('to_do_list_reciepent')->where('reciepent_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update([
    'reciepent_pro_pic_path' => 'https://todosmarter.com/images/to_do_smarter_img/' . $imageName
    ]);
 ...
 public function monthlyBreakdownView(){
  ...
  if (!isset($_COOKIE['UserFullNameToDoSmarter'])) {
    ...
    $months = DB::table("todo_job_months")->orderBy("m_month", "desc")->groupBy("m_month")->get();
       ...
       foreach($months as $m) {
            $data = DB::table("to_do_list_new")->where("todo_month", "=", $m->m_month)->where("to_do_list_status", "!=", "1")->where('to_do_list_job_status', '=', '1')->where('to_do_list_add_id', '=', Crypt::decryptString($_COOKIE['UsersCompanyToDoSmarter']))->orderBy("to_do_list_id", "desc")->get();
    ...
    return view('admin.monthly_breakdown')->with(['data'=>$arr]);
 }
 ...
 public function makeColorPost(Request $request){

  DB::table('to_do_list_reciepent')->where('reciepent_id', Crypt::decryptString($_COOKIE['UserIdToDoSmarter']))->update([
    'reciepent_color_pref' => $request->input('color')
    ]);
...
try {
  DB::table('suggstn_box')->insert([
      'sggstn_author' => Crypt::decryptString($_COOKIE['UserFullNameToDoSmarter']),
      'sggstn_author_id' => Crypt::decryptString($_COOKIE['UserIdToDoSmarter']),
      'sggstn_web_name' => request('web_name'),
      'sggstn_comment' => request('comment'),
      'sggstn_room_name' => 'tds'
  ]);
