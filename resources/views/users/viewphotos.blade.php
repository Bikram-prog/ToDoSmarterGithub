                        ...
                        <!-- <span class="text-warning bg-orange-darker rounded px-2 py-1" rel="tooltip" title="This photo must be approved by administration">
                            Not verified                        </span> -->
                                ...
                                <div class="ml-auto">
                                    ...
                                    @if($pic->user_photo_path != $pic->prfl_photo_path && $pic->approve_status == '1')
                                    ...
                                    <p style="margin-top: 5px; font-weight: 700;font-style: italic;"><i class="fas fa-user-clock"></i>Your photo was successfully uploaded and is now pending review.All photos are reviewed by our customer satisfaction team and should be processed within 24hours.</p>
                                    @endif
                                	<!-- @if($pic->private_status == 0)
                                                              <a class="btn btn-sm btn-success" href="/privatephoto/{{ $pic->user_photo_id }}" rel="tooltip" ><i class="fas fa-lock-open"></i></a>

                                    @else
                                                              <a class="btn btn-sm btn-success" href="/unlockphoto/{{ $pic->user_photo_id }}" rel="tooltip" ><i class="fas fa-lock"></i></a>
                                    @endif -->

                                                              <a onclick="search()" class="btn btn-sm btn-danger" href="/deletephoto/{{ $pic->user_photo_id}}/{{ $picture }}" ><i class="fas fa-trash"></i></a>                </div>
            </div>
        </div>
    </div>
</div>
